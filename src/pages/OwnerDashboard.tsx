...
import { TransformerStatus, AnomalyResult } from '@/lib/types';
...
const OwnerDashboard = () => {
  ...
  const criticalCount = Object.values(allStatuses).flat().filter(t => t.riskLevel === 'CRITICAL').length;
  const highCount = Object.values(allStatuses).flat().filter(t => t.riskLevel === 'HIGH').length;
  
  const avgUtilization = Object.values(allStatuses).flat().length > 0
    ? Object.values(allStatuses).flat().reduce((sum, t) => sum + t.utilizationPercent, 0) / Object.values(allStatuses).flat().length
    : 0;

  const carbonSaved = 12.4; // Mock
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Multi-society energy monitoring overview</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <StatCard title="Carbon Saved" value={`${carbonSaved}T`} icon={Leaf} subtitle="This month" trend={{ value: 8, positive: true }} />
      </div>

      {/* Risk overview */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <RiskCard title="Blackout Probability" value={criticalCount > 0 ? 23.5 : 4.2} unit="%" riskLevel={overallRisk} subtitle="Based on current load patterns" />
        <RiskCard title="Anomaly Rate" value={highCount + criticalCount} unit="alerts" riskLevel={highCount > 1 ? 'HIGH' : 'NORMAL'} subtitle="Across all societies" />
        <RiskCard title="Grid Stability" value={100 - avgUtilization} unit="% headroom" riskLevel={avgUtilization > 80 ? 'CRITICAL' : avgUtilization > 65 ? 'HIGH' : 'NORMAL'} />
      </div>

      {/* Society risk heatmap */}
      <div className="bg-card rounded-lg border border-border p-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-5 gap-3">
          {SOCIETIES.map(s => {
            ...
            const worstRisk = statuses.find(t => t.riskLevel === 'CRITICAL')
              ? 'CRITICAL' : statuses.find(t => t.riskLevel === 'HIGH')
              ? 'HIGH' : 'NORMAL';
            ...
            return (
              <motion.div
                ...
                className={`p-4 rounded-lg border cursor-pointer transition-all ${
                  selectedSociety === s.id ? 'border-primary bg-primary/5' : 'border-border bg-secondary/30'
                } ${worstRisk === 'CRITICAL' ? 'glow-critical' : worstRisk === 'HIGH' ? 'glow-warning' : ''}`}
              >
                ...
                <div className="flex items-center justify-between mt-3">
                  ...
                  <span className={`text-xs font-semibold px-2 py-0.5 rounded ${
                    worstRisk === 'CRITICAL' ? 'bg-critical/20 text-critical' :
                    worstRisk === 'HIGH' ? 'bg-warning/20 text-warning' :
      ...
      <div>
        <h3 className="text-sm font-medium text-foreground mb-3">
          {SOCIETIES.find(s => s.id === selectedSociety)?.name} — Transformers
        </h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {selectedStatuses.map(t => (
